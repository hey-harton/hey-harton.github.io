...
export default function ValuesSection() {
  ...
  return (
    // PERBAIKAN: Menghapus transform-gpu
    <section className="w-full bg-transparent border-b border-white/10 relative z-10">
      <div className="max-w-[1600px] mx-auto px-6 md:px-12 lg:px-16 py-24 lg:py-32">
        ...
        <div className="mb-16 border-l border-brand-blue pl-6 md:pl-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-10">
          {values.map((val, index) => (
            <div 
              key={index} 
              className="group relative flex flex-col bg-[#050505] border border-white/10 p-8 md:p-10 transition-colors duration-300 rounded-none hover:bg-[#0a0a0e]"
            >
              ...
              {/* Judul Prinsip */}
              <h3 className="text-lg md:text-xl font-bold text-white mb-6 tracking-tight uppercase group-hover:text-brand-blue transition-colors">
                {val.title}
              </h3>
              
              {/* Deskripsi Teknis (Font-Mono) */}
              <p className="text-gray-400 font-mono text-xs md:text-sm leading-relaxed font-light text-left mt-auto">
                {val.desc}
              </p>
