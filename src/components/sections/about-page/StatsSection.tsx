export default function StatsSection() {
  ...
  return (
    <section className="w-full border-b border-white/10 bg-transparent relative z-10">
      {/* PERBAIKAN: Padding vertikal (py) disesuaikan untuk HP agar tidak terlalu berongga */}
      <div className="max-w-[1600px] mx-auto px-6 md:px-12 lg:px-16 py-16 md:py-24 lg:py-32">
        ...
        <div className="bg-white/10 gap-[1px] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 border border-white/10">
          {stats.map((stat) => (
            // PERBAIKAN: Padding diubah menjadi p-6 sm:p-8 md:p-10 untuk responsivitas layar kecil
            <div key={stat.id} className="bg-[#050505] p-6 sm:p-8 md:p-10 flex flex-col h-full relative group hover:bg-[#0a0a0e] transition-colors">
              ...
              <span className="text-gray-600 font-mono text-[8px] md:text-[9px] uppercase tracking-widest mb-3 md:mb-4 block">
                / {stat.id}
              </span>

              {/* Nilai Utama */}
              <span className="text-3xl md:text-4xl lg:text-5xl font-mono font-bold text-white mb-2 tracking-tight">
                {stat.value}
              </span>
              
              {/* Label Kategori */}
              <span className="text-brand-blue font-mono font-bold text-[9px] md:text-[10px] uppercase tracking-widest mb-4 md:mb-6 block border-b border-white/5 pb-2 md:pb-3">
                {stat.label}
              </span>
              
              {/* Deskripsi (Rata Kiri) */}
              <p className="text-gray-500 text-[10px] md:text-xs lg:text-sm leading-relaxed font-mono font-light mt-auto text-left">
                {stat.desc}
              </p>
