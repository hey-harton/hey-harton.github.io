...
export default async function UpdateHistorySection({ githubUrl, updates = [] }: UpdateHistoryProps) {
  ...
  return (
    // PERBAIKAN: Margin disesuaikan untuk mobile (mb-16 md:mb-24)
    <section className="w-full px-4 md:px-12 lg:px-16 max-w-[1600px] mx-auto mb-16 md:mb-24 pt-12 md:pt-16 border-t border-white/10 relative">
      ...
      <div className="max-w-4xl relative z-10 pl-0 lg:pl-10">
        ...
        <div className="mb-10 md:mb-14">
          ...
          <h2 className="text-2xl md:text-4xl font-bold text-white mb-4 uppercase tracking-tight break-words">
            Siklus Log Proyek<span className="text-brand-blue">_</span>
          </h2>
          <p className="text-xs md:text-base text-gray-500 font-mono tracking-wide leading-relaxed">
            {hasGithubCommits 
        ...
        {/* Timeline Updates */}
        <div className="relative border-l border-white/10 ml-2 md:ml-4 space-y-8 md:space-y-10 pb-4">
          {timelineData.map((item) => (
            <div key={item.id} className="relative pl-6 md:pl-12 group">
              
              {/* Node / Titik Timeline */}
              {/* PERBAIKAN: Menghapus efek shadow glow untuk menyelamatkan FPS saat scrolling */}
              <div className="absolute -left-[4.5px] top-2.5 w-2 h-2 border border-brand-blue bg-[#050505] group-hover:bg-brand-blue transition-colors duration-300 rounded-none"></div>
              
              {/* Meta Data: Version & Date */}
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 mb-4">
                <span className="flex items-center gap-2 text-brand-blue font-mono font-bold border border-brand-blue/30 bg-brand-blue/5 px-3 py-1 text-[9px] md:text-[10px] uppercase w-max tracking-widest group-hover:border-brand-blue transition-colors rounded-none">
                  <GitCommit size={14} />
                  {item.version}
                </span>
                <div className="flex items-center gap-2 text-gray-500 text-[9px] md:text-[10px] uppercase font-mono tracking-widest">
              ...
              <div className="bg-[#050505] border border-white/5 p-4 md:p-6 group-hover:border-brand-blue/30 transition-all duration-300 flex flex-col gap-4 md:gap-5 rounded-none relative">
                ...
                <p className="text-gray-400 font-mono text-[10px] md:text-xs lg:text-sm font-light text-left leading-relaxed">
                  {item.description}
                </p>
                ...
                {item.link && (
                  <Link 
                    href={item.link}
                    target="_blank"
                    className="inline-flex items-center gap-2 text-[9px] md:text-[10px] font-mono font-bold uppercase tracking-widest text-gray-500 hover:text-brand-blue transition-colors w-max pb-1 border-b border-transparent hover:border-brand-blue"
                  >
